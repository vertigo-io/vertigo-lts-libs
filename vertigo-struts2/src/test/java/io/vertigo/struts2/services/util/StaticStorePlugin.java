...
public final class StaticStorePlugin extends AbstractStaticEntityStorePlugin implements Activeable {
	...
	public void start() {
		...
		final BasicType keyDataType = staticDtDefinition.getIdField().get().smartTypeDefinition().getBasicType();
	...
	public <E extends Entity> DtList<E> findByCriteria(final DtDefinition dtDefinition, final Criteria<E> criteria, final DtListState dtListState) {
		Assertion.check()
				...
				.isTrue(criteria == null || criteria == Criterions.<E> alwaysTrue(), "This store could only load all data, not {0}", criteria);
		//----
		return (DtList) dtc;
	}
